...
lerobot-train \
 ...
 '--dataset.image_transforms.tfs={"identity":{"type":"Identity","weight":0.8,"kwargs":{}},"color_jitter":{"type":"ColorJitter","weight":0.2,"kwargs":{"brightness":[0.5,1.5],"contrast":[0.8,1.2],"saturation":[0.8,1.2],"hue":[-0.03,0.03]}}}' \
  --dataset.transforms_refresh=0 # to confirm
  --dataset.masks_dir="DerBoroter/full_fold_tristan_300eps/masks" \
  --dataset.recolor_prob=0.8 \
  --dataset.recolor_hue_range="[0,120]" \
  --dataset.recolor_sat_range="[150,220]" \
  --dataset.recolor_val_range="[0.9,1.1]" \
  --dataset.recolor_debug_dir="outputs/debug_images" # to remove for full training
